...
public class Custom {
    ...
    public Button clear,inButton,preButton,postButton;
    ...
    public void initialize() throws IOException {
        ...
        dialog.setTitle("Enter the root");
        dialog.setHeaderText("In order to initialize the tree i need the root");
        dialog.setContentText("Please enter the root:");
        Optional<String> result = dialog.showAndWait();
        if(!result.isPresent())
            switchToMenu(new ActionEvent());
    ...
    public void update(){
        ...
        int shift;
        setLabels();
        pane.getChildren().removeIf(node -> node instanceof Circle || node instanceof Label|| node instanceof Line|| node instanceof ImageView);
        if(tree.root!=null){
                ...
                for (int j = 0; j < arr[0].length; j++)
                    if (nodes[j] != null) {
                        ...
                        if(nodes[j].right==null){
                            right=new ImageView(new Image("/MainApp/assets/rightArrow.png"));
                            right.setTranslateX(nodes[j].pos[0]+15);
                            right.setTranslateY(nodes[j].pos[1]+25);
                            right.setFitWidth(20);
                            right.setFitHeight(20);
                            int finalJ = j;
                            right.setOnMouseClicked(event -> getNode(nodes[finalJ].key,true));
                            pane.getChildren().add(right);
                        }else
                            pane.getChildren().addAll(new Line(nodes[j].pos[0],nodes[j].pos[1] , nodes[j].right.pos[0]+shift, nodes[j].right.pos[1]));

                        if(nodes[j].left==null){
                            left=new ImageView(new Image("/MainApp/assets/leftArrow.png"));
                            left.setTranslateX(nodes[j].pos[0]-35);
                            left.setTranslateY(nodes[j].pos[1]+25);
                            left.setFitWidth(20);
                            left.setFitHeight(20);
                            int finalJ = j;
                            left.setOnMouseClicked(event -> getNode(nodes[finalJ].key,false));
    ...
    public void getNode(String parent,boolean isRight){
        TextInputDialog dialog = new TextInputDialog();
        dialog.setTitle("Enter the node");
        dialog.setContentText("Please enter the node:");
        Optional<String> result = dialog.showAndWait();
        result.ifPresent(child -> {
            if(!tree.postorder().contains(child))
                tree.insert(child,parent,isRight);
            update();
        });
    }
